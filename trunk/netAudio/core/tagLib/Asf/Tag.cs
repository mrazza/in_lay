...
#region IEnumerable
		...
		///    Content Descriptor.
        ///    
        ///    This property has been hacked to support both TrackNumber and TrackCount together.
		/// </remarks>
			...
			get {
				foreach (ContentDescriptor desc in
					GetDescriptors ("WM/TrackNumber")) {
                        ...
                        int loc = -1;
                        uint value;
                    string sVersion = desc.ToString();
                    if ((loc = sVersion.IndexOf('/')) > -1)
                    {
                        uint value2;
                        value = uint.Parse(sVersion.Substring(0, sVersion.IndexOf('/')));
                        value2 = uint.Parse(sVersion.Substring(sVersion.IndexOf('/') + 1));
                        if (TrackCount == 0)
                            TrackCount = value2;
